    ...
    public sealed partial class EntityComponentsHolder : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            for (int i = 0; i < _components.Count; i++)
                (_components as IEntityComponentInitializer)?.Initialize(this);
        }

        private void Update()
        {
            for (int i = 0; i < _components.Count; i++)
                (_components as IEntityComponentUpdater)?.Update(this);
        }

        private void OnDisable()
        {
            for (int i = 0; i < _components.Count; i++)
                (_components as IEntityComponentCleaner)?.Clear(this);
        }
    }
